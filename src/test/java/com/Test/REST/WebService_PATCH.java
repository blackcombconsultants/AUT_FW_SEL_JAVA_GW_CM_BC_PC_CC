...
public class WebService_PATCH {
	...
	public void rest_patch_json_cucumber_Test01() {
		...
		given().header("Content-Type", "application/json").contentType(ContentType.JSON).accept(ContentType.JSON).body(oJSON.toJSONString()).when().patch(strURl).then().statusCode(200).log().all();
	...
	public void rest_patch_json_mockserver_cucumber_Test01() {
		...
		given().header("Content-Type", "application/json").contentType(ContentType.JSON).accept(ContentType.JSON).body(oJSON.toJSONString()).when().patch("/Department/6").then().statusCode(200).log().all();
	}
}
